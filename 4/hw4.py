...
class MountainCarQLearningAgent:
    ...
    def __init__(self, alpha=0.5, epsilon=1, gamma=0.95, max_steps_per_episode=1000,
        ...
        "*** YOUR CODE HERE ***"
        "self.qvalues[last_state][action][next_state]"
        self.qvalues = [[0  for j in range(self.action_number)] for i in range(self.state_number * 2)]
        self.sample_count = [[0 for j in range(self.action_number)] for i in range(self.state_number  * 2)] 
        "*** END YOUR CODE ***"
    ...
    def get_policy_action(self, state):
        ...
        for a in range(self.action_number):
            if self.qvalues[state][a] > self.qvalues[state][action]:
                action = a
        "*** END YOUR CODE ***"
        return action
    ...
    def update(self, state, action, next_state, reward):
        "Update your qvalues"
        "*** YOUR CODE HERE ***"
        next_max_value = self.qvalues[next_state][0]
        for value in self.qvalues[next_state]:
            next_max_value = max(next_max_value,value)

        total_sum = self.qvalues[state][action] * self.sample_count[state][action] + reward + next_max_value
        self.sample_count[state][action] += 1
        self.qvalues[state][action] = total_sum / self.sample_count[state][action]
        "*** END YOUR CODE ***"
    ...
    def train_one_episode(self):
        ...
        for _ in range(self.max_steps_per_episode):
            ...
            self.update(state,action,self.observation_to_state(ob),reward)
            if done:
                return
            "*** END YOUR CODE ***"
            steps += 1
    ...
    def train(self):
        ...
        for i in range(self.num_episode):
            ...
            print('{} episode is finished {}'.format(i,self.epsilon))
        "*** END YOUR CODE ***"
