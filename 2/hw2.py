...
def formula2d():
    ...
    return Forall('$x',Forall('$y',Forall('$z',Equiv(And(Female('$x'),And(Parent('$y','$x'),Parent('$z','$y'))),Grandmother('$z','$x')))))
    # END_YOUR_CODE
...
# In particular, Equals(x,x) = True and Equals(x,y) = False iff x is not equal to 
def liar():
    ...
    def OnlyWho(x,exp):
        return And(IfItIsWhoThen(x,john,exp),And(IfItIsWhoThen(x,susan,exp),And(IfItIsWhoThen(x,nicole,exp),IfItIsWhoThen(x,mark,exp))))
    
    def OnlyOne(exp):
        return Or(OnlyWho(john,exp),Or(OnlyWho(susan,exp),Or(OnlyWho(mark,exp),OnlyWho(nicole,exp))))
